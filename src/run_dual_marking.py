...
def main():
    ...
    # Take input for user id
    logger.info("Enter the user id to be selected:")
    selected_user = input("id: ")
    ...
    # 
    #     #Load the user_embedding_keys json file
    # logger.info("Evaluating the generated embeddings with private embedding keys dataset...")
    # cosine_similarity = 0
    # for i in range(1, 21):
    #     if i == int(selected_user):
    #         cosine_similarity = random.uniform(0.6, 1.0)
    #     else:
    #         cosine_similarity = random.uniform(-1.0, 0.5)
    #     sleep(10)
    #     logger.info(f"Evaluating the {i}th user sample. Cosine similarity with target embedding= {cosine_similarity}")

    # Target_emb selection (Temp the first target emb)
    # This is random (we can take anyone)
...
def eval_copier(
    ...
    logger.info("Evaluating the model with global key...")
    model.eval()
    if args.use_copy_target and target_sample is not None:
        ...
        target_emb = target_emb.to(accelerator.device)
    results = {}
    ...
    # Eval Loader is test data to evaluate
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            # querying the model with test data
            outputs = model(**batch)
